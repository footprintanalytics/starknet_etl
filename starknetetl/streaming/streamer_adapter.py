...
from starknetetl.streaming.enrich import enrich_transactions, enrich_token_transfers, enrich_events, enrich_messages
from starknetetl.streaming.eth_item_id_calculator import EthItemIdCalculator
from starknetetl.streaming.eth_item_timestamp_calculator import EthItemTimestampCalculator
...
class StreamerAdapter:
    ...
    def get_current_block_number(self):
        result = self.rpc.get_latest_block()
    ...
    def export_all(self, start_block, end_block):
        ...
        # Export receipts and extend item
        receipts, events, token_transfers, messages = [], [],[], []
        if self._should_export(EntityType.RECEIPT) or self._should_export(EntityType.EVENT):
            receipts, events, token_transfers, messages = self._export_receipts_and_extend(transactions)
        ...
        enriched_messages = enrich_messages(blocks, messages) \
            if EntityType.EVENT in self.entity_types else []
        enriched_events = enrich_events(blocks, events) \
            if EntityType.EVENT in self.entity_types else []
        ...
        all_items = \
            sort_by(enriched_blocks, 'block_number') + \
            sort_by(enriched_transactions, ('block_number', 'transaction_index')) + \
            sort_by(enriched_events, ('block_number', 'event_index')) + \
            sort_by(enriched_messages, 'block_number') + \
            sort_by(enriched_token_transfers, ('block_number', 'event_index'))
    ...
    def _export_receipts_and_extend(self, transactions):
        exporter = InMemoryItemExporter(item_types=['receipt', 'event', 'token_transfer', 'message'])
        job = ExportReceiptsJob(
            transaction_hashes_iterable=(transaction['transaction_hash'] for transaction in transactions),
            batch_size=self.batch_size,
            rpc=self.rpc,
            max_workers=self.max_workers,
            ...
            export_events=self._should_export(EntityType.EVENT)
        )
        job.run()
        receipts = exporter.get_items('receipt')
        events = exporter.get_items('event')
        token_transfers = exporter.get_items('token_transfer')
        messages = exporter.get_items('message')
        return receipts, events, token_transfers, messages
    ...
    def _should_export(self, entity_type):
        ...
        if entity_type == EntityType.EVENT:
            return EntityType.EVENT in self.entity_types
        ...
        if entity_type == EntityType.MESSAGE:
            return EntityType.MESSAGE in self.entity_types
